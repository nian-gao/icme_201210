...
class UnalignedDataset(BaseDataset):
    ...
    It requires two directories to host training images from domain B '/path/to/data/trainA'
    and from domain ori '/path/to/data/trainB' respectively.
    ...
    def __init__(self, opt):
        ...
        self.dir_B = os.path.join(opt.dataroot,  'B/' + opt.phase)  # create a path '/path/to/data/trainB'
    ...
    def __getitem__(self, index):
        ...
        Returns a dictionary that contains B, ori, A_paths and B_paths
            B (tensor)       -- an image in the input domain
            ori (tensor)       -- its corresponding image in the target domain
        ...
        else:   # randomize the index for domain ori to avoid fixed pairs.
        ...
        E = self.transform_E(E_img)
        M = self.transform_M(M_img)
        B = self.transform_B(B_img)

        # return {'B': A, 'ori': B, 'A_paths': A_path, 'B_paths': B_path}
        return {'C': C, "E": E, "M": M, "B": B,
                "C_paths": C_path, "E_paths": E_path, "M_paths": M_path, "B_paths": B_path}
